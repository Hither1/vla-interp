...
@torch.no_grad()
def compute_feature_mean_var_streaming(X: torch.Tensor, chunk_size: int = 200_000):
    """
    Streaming mean/var over rows for featurewise normalization.
    X: [N, D] on CPU (float32 recommended).
    Returns mean [D], var [D] on CPU float32.
    """
    ...
    for start in range(0, N, chunk_size):
        xb = X[start:start + chunk_size].to(dtype=torch.float64)
        sum_ += xb.sum(dim=0)
        sumsq += (xb * xb).sum(dim=0)
        count += xb.shape[0]

    mean = sum_ / max(count, 1)
    var = (sumsq / max(count, 1)) - mean * mean
    var = torch.clamp(var, min=0.0)  # numerical safety
    return mean.to(dtype=torch.float32), var.to(dtype=torch.float32)
