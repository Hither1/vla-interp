...
all_embeds = []  # list to hold each file's activations

for npy_path in npy_paths:
    print(f"Processing {npy_path}")
    embeds = np.load(npy_path)  # expected shape: (N_i, num_layers, d)
...
print("Concatenated activations shape:", Activations_np.shape)
...
# ---- side-by-side plots for Loss and R² ----
# fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# ---- Loss (left) ----
# ax = axes[0]
# for layer_idx, loss_history in all_loss_logs.items():
#     ax.plot(loss_history, label=f"Layer {layer_idx}")
# ax.set_xlabel("Training step")
# ax.set_ylabel("Loss")
# ax.set_title("SAE Training Loss per Layer")
# ax.legend()
...
for layer_idx, r2_history in all_r2_logs.items():
    color = cmap(layer_idx) 
    plt.plot(r2_history, color=color, label=f"Layer {layer_idx}")
